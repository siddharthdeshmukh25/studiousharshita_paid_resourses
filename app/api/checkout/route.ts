    ...
    if (settings.gateway === 'razorpay') {
      ...
      const razorpayResponse = await fetch(`${razorpayBaseUrl()}/orders`, {
        ...
        body: JSON.stringify({
          ...
          notes: {
            ...
            merchantName: 'Studious Harshita', // Business branding to reduce payment rejection flags
      ...
      const cashfreeResponse = await fetch(`${cashfreeBaseUrl(settings)}/orders`, {
        ...
        body: JSON.stringify({
          ...
          order_note: `Purchase: ${resource.title} — Studious Harshita (educational digital resource)`,
