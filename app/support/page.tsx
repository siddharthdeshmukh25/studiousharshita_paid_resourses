      ...
      <div className="min-h-screen flex flex-col bg-[var(--background)]">
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <h1 className="text-2xl font-bold text-[#1A1A1A] mb-2">Support Center</h1>
    ...
    <div className="min-h-screen flex flex-col bg-[var(--background)]">
      ...
      <main className="flex-1 py-8 md:py-12">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header */}
          <div className="mb-8">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--accent)]">Support Center</p>
            <h1 className="mt-1 text-2xl md:text-3xl font-bold text-[#1A1A1A] flex items-center gap-3">
              <LifeBuoy className="h-7 w-7 text-[var(--accent)]" />
              How can we help?
            </h1>
          ...
          {/* Create ticket */}
          <section className="bg-[#FFFDF8] rounded-2xl border border-[var(--line)] shadow-sm p-6 md:p-8 mb-8">
            <h2 className="text-lg font-semibold text-[#1A1A1A] flex items-center gap-2 mb-1">
            ...
            {success && (
              <div className="mb-5 p-4 bg-[var(--accent-soft)] border border-[var(--accent-soft-2)] rounded-lg text-[var(--accent-deep)]">{success}</div>
            )}

            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="ticket-subject" className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                  Subject
                </label>
                <input
                  id="ticket-subject"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  <label htmlFor="ticket-category" className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                    Category
                  </label>
                  <select
                    id="ticket-category"
                ...
                <div>
                  <label htmlFor="ticket-order" className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                    Order ID <span className="font-normal text-[#6B6257]">(optional)</span>
                  </label>
                  <input
                    id="ticket-order"
                    value={orderId}
              ...
              <div>
                <label htmlFor="ticket-message" className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                  Describe your issue
                </label>
                <textarea
                  id="ticket-message"
          ...
          {/* My tickets */}
          <section>
            <h2 className="text-lg font-semibold text-[#1A1A1A] mb-4">My tickets</h2>

            {loading ? (
              <div className="bg-[#FFFDF8] rounded-2xl border border-[var(--line)] p-10 text-center">
                <Loader2 className="h-7 w-7 animate-spin text-[#A29785] mx-auto" />
              </div>
            ) : error ? (
              <div className="bg-[#FFFDF8] rounded-2xl border border-[var(--line)] p-8 text-center text-red-600">{error}</div>
            ) : tickets.length === 0 ? (
              <div className="bg-[#FFFDF8] rounded-2xl border border-[var(--line)] p-10 text-center">
                <p className="text-base text-[#6B6257]">You have no support tickets yet.</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-1.5">
                            <span className="text-xs font-semibold text-[#6B6257]">
                              {new Date(ticket.createdAt).toLocaleDateString('en-IN', {
                                day: 'numeric', month: 'short', year: 'numeric',
                              })}
                            </span>
                            {ticket.orderId && (
                              <span className="text-xs font-mono text-[#6B6257]">#{ticket.orderId}</span>
                            )}
                          </div>
                          <h3 className="text-base font-semibold text-[#1A1A1A] leading-snug">{ticket.subject}</h3>
                          {lastMessage && (
                            <p className="mt-1 text-sm text-[#6B6257] line-clamp-1">{lastMessage.text}</p>
                          )}
                        </div>
                        <div className="flex flex-col items-end gap-2 flex-shrink-0">
                          ...
                          <ChevronRight className="h-5 w-5 text-[#A29785]" />
                        </div>
                      </div>
                    </button>
                  );
...
export default function SupportPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex flex-col bg-[var(--background)]"><Navbar /></div>}>
      <SupportPageContent />
    </Suspense>
  );
}