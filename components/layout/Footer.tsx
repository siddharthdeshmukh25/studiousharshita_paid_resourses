...
import { SITE_NAME, TELEGRAM_CHANNEL_URL, WHATSAPP_CHANNEL_URL } from '@/lib/site';
...
export default function Footer() {
  ...
  // Community channels (lib/site.ts defaults) — empty entries are hidden.
  const CHANNEL_LINKS = [
    { href: TELEGRAM_CHANNEL_URL, label: 'Telegram Channel' },
    { href: WHATSAPP_CHANNEL_URL, label: 'WhatsApp Channel' },
  ...
  return (
    <footer className="border-t border-[#1A1A1A] bg-[#1A1A1A] text-[#FAF6EF]">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <div className="grid grid-cols-2 gap-6 sm:gap-8 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr]">
          {/* Brand */}
          <div className="col-span-2 sm:col-span-2 lg:col-span-1">
            <p className="font-serif-display text-xl tracking-tight sm:text-2xl">
              studious<span className="italic text-[var(--butter)]">harshita</span><span className="text-[var(--butter)]">.</span>
            </p>
            <p className="mt-2 max-w-xs text-[13px] leading-5 text-[#FAF6EF]/60 line-clamp-2 sm:line-clamp-none">
              Study resources, productivity systems and digital products for ambitious students across India.
            </p>
            <p className="font-hand mt-2.5 text-base text-[var(--butter)] sm:text-lg">made for students ♡</p>
          </div>
              ...
              {CHANNEL_LINKS.map((link) => (
                <li key={link.label}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-semibold text-[var(--butter)] transition-colors hover:text-[#FAF6EF]"
                  >
                    {link.label} <span aria-hidden="true">↗</span>
                  </a>
                </li>
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2.5 text-[13px] text-[#FAF6EF]/80">
              {CONTACT_EMAILS.map(({ email, purpose }) => (
                <li key={email} className="flex items-start gap-2.5">
                  <Mail className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[var(--butter)]" />
                  <span>
                    <a href={`mailto:${email}`} className="break-all transition-colors hover:text-[var(--butter)]">
                      {email}
                    </a>
                    <span className="mt-0.5 block text-[10px] uppercase tracking-[0.1em] text-[#FAF6EF]/45">
                      {purpose}
                    </span>
                  </span>
                </li>
        ...
        {/* Bottom bar */}
        <div className="mt-6 flex flex-col gap-2.5 border-t border-[#FAF6EF]/15 pt-4 sm:mt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="font-hand text-base text-[#FAF6EF]/70 sm:text-lg">Made with curiosity, coffee &amp; too many tabs open.</p>
        ...
        <p className="mt-2 text-center text-xs text-[#FAF6EF]/35 sm:text-right">
