...
/**
 * Editorial body sections for the home page: welcome intro, magazine profile,
 * digital-product shop (live from the API), community quotes, FAQ, newsletter
 * signup and the closing CTA. All copy follows the editorial Studygram voice.
 */
...
export default function HomeSections() {
  ...
  useEffect(() => {
    ...
    fetch('/api/stats')
      .then((res) => res.json())
      .then((data: { reviews?: StatReview[]; totalStudents?: number }) => {
        if (!cancelled) {
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:px-8">
          ...
          <Reveal delay={120}>
            <div aria-hidden="true" className="pointer-events-none relative mx-auto h-72 w-full max-w-md select-none sm:h-80">
              ...
              <div className="absolute bottom-2 left-1/2 w-48 -translate-x-1/2 rotate-[-2deg] rounded-lg bg-[var(--butter)] p-4 shadow-[0_12px_28px_rgba(26,26,26,0.12)]">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-12 lg:gap-12 lg:px-8">
          <Reveal className="lg:col-span-5">
            ...
            <div aria-hidden="true" className="pointer-events-none relative mx-auto h-80 w-72 select-none sm:h-96 sm:w-80">
              ...
              <span className="tape absolute -top-3 left-1/2 -translate-x-1/2 rotate-[3deg]" />
              <div className="absolute inset-5 grid place-items-center rounded-md bg-[var(--sage-soft)]">
                <div className="text-center">
                  <span className="sticker-dot mx-auto !h-14 !w-14 !text-2xl">✷</span>
                  <p className="font-serif-display mt-4 text-3xl italic text-[#1A1A1A]">Harshita</p>
                  <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[#6B6257]">student · creator · maker</p>
                </div>
              </div>
              <span className="font-hand absolute -bottom-5 left-2 rotate-[-6deg] text-xl text-[#6B6257]">that&apos;s me → keeping it real since day one</span>
            </div>
          </Reveal>

          <Reveal delay={100} className="lg:col-span-7">
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#6B6257]">the girl behind the notes</p>
            <h2 className="font-serif-display mt-1 text-3xl italic text-[#1A1A1A] sm:text-4xl">Hi, I&apos;m Harshita.</h2>
            <p className="mt-4 max-w-xl text-sm leading-7 text-[#4A443B] sm:text-base sm:leading-8">
              I&apos;m a student and content creator building Studious Harshita as a space for ambitious
              students who want to learn, create and grow. What started as messy notebooks shared with
              friends is now a small library of exam-ready notes, planners and tools.
            </p>
            ...
            <dl className="mt-7 grid grid-cols-2 gap-3 sm:max-w-lg">
              {[
                ...
                { value: `${Math.max(totalStudents ?? 0, 100)}+`, label: 'students here' },
                { value: 'global', label: 'student audience' },
              ].map((stat, index) => (
                <div key={stat.label} className={`rounded-lg border border-[var(--line)] p-4 ${['bg-[#FFFDF8]', 'bg-[var(--blush-soft)]', 'bg-[var(--sky-soft)]', 'bg-[var(--butter-soft)]'][index]}`}>
              ...
              {shopItems.map((item, index) => (
                <Reveal key={item._id} delay={index * 60}>
                  <Link href={`/resource/${item._id}`} className="group block h-full">
                    ...
                    <div className={`relative rounded-lg border border-[var(--line)] ${SHOP_BLOCK_STYLES[index % SHOP_BLOCK_STYLES.length].bg} p-4 transition-all duration-300 group-hover:-translate-y-1.5 group-hover:shadow-[0_14px_28px_rgba(26,26,26,0.12)]`}>
                      ...
                      <div className="flex h-24 items-center justify-center rounded-md bg-[#FFFDF8]/85 shadow-inner">
                        {item.thumbnailUrl ? (
                          ...
                          <img src={item.thumbnailUrl} alt={item.title} loading="lazy" className="h-full w-full rounded-md object-cover" />
                        ) : (
            ...
            {testimonials.map((review, index) => (
              <Reveal key={`${review.userName}-${index}`} delay={index * 70}>
                <figure className="flex h-full flex-col rounded-2xl border border-[var(--line)] bg-[#FFFDF8] p-5 shadow-[0_6px_18px_rgba(26,26,26,0.05)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_24px_rgba(26,26,26,0.09)]">
                  ...
                  <blockquote className="mt-3 flex-1 text-sm leading-6 text-[#3F3A32]">“{review.comment}”</blockquote>
